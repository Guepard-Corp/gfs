# D3: you deleted, the source updated: the row stays deleted
...
```sql
UPDATE orders SET total=777 WHERE id=1;
```

## What is asserted

- the row is not resurrected
- pull reports it rather than silently reviving it

## Running it

```bash
tests/paths/run-all.sh D3          # through the runner
bash tests/paths/D3_local_delete_source_update/test.sh   # directly
```
