# B2: source UPDATE is visible to the clone
...
- returns the new value, not the cached 30

## Running it

```bash
tests/paths/run-all.sh B2          # through the runner
bash tests/paths/B2_row_changed/test.sh   # directly
```
